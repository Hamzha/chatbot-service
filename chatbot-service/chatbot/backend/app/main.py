...
# Browser UI (Next.js) runs on another origin (port/hostname). Without CORS,
# POST multipart uploads from the UI fail in the browser as "Failed to fetch".
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def get_job(event_id: str):
    ...
    # Keep dependency surface small by using stdlib event loop + requests thread.
    import requests
