...
export default async function DashboardPage() {
    ...
    return (
        <PageContainer>
            <header className="glass-strong relative overflow-hidden rounded-2xl p-5 sm:p-6 lg:p-8">
                <div className="relative z-10 flex flex-col items-start gap-5 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0 space-y-2">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-700">Dashboard · Overview</p>
                        <h1 className="text-2xl font-semibold leading-tight tracking-tight text-slate-900 sm:text-3xl">
                            Welcome back, {firstName}
                        </h1>
                        <p className="max-w-2xl text-sm leading-6 text-slate-700">
                            Here is your three-step workflow for turning raw web pages and documents into reliable chatbot answers. Pick up where you left off, or jump straight into the chatbot.
                        </p>
                    </div>
                    <Link
                        href="/dashboard/chatbot"
                        className="inline-flex w-full shrink-0 items-center justify-center gap-2 rounded-xl bg-brand-700 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-brand-700/20 transition hover:bg-brand-800 sm:w-auto"
                    >
                        Open chatbot
            ...
            <section aria-labelledby="pipeline-heading">
                <div className="mb-3 flex items-center justify-between px-1">
                    <h2 id="pipeline-heading" className="text-sm font-semibold uppercase tracking-wide text-slate-700">
                        Your pipeline
                    </h2>
                    <span className="text-xs text-slate-600">3 steps</span>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 sm:gap-5">
                    {PIPELINE.map((item, i) => (
                        <Link
                            key={item.step}
                            href={item.href}
                            className="glass-strong group relative flex flex-col rounded-2xl p-5 transition-all hover:-translate-y-0.5 hover:shadow-xl sm:p-6"
                        >
                            <div className="mb-4 flex items-center justify-between sm:mb-5">
                                ...
                                {i < PIPELINE.length - 1 && (
                                    <span className="hidden text-slate-400 md:inline-flex" aria-hidden="true">
            ...
            <section className="grid gap-4 sm:gap-5 lg:grid-cols-[1.5fr_1fr]">
                <div className="glass-strong rounded-2xl p-5 sm:p-6">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-700">Getting started</p>
                ...
                <aside className="glass-dark flex flex-col rounded-2xl p-5 sm:p-6">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-200">Tips</p>
    ...
    return (
        <li className="flex gap-4">
            ...
            <div className="min-w-0">
                ...
                <p className="mt-0.5 text-sm leading-6 text-slate-700">{body}</p>
            </div>
        </li>
    );
}
